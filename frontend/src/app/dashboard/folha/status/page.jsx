...
function ProcessamentoCard({ processamento, baixandoArquivo, onBaixar, tentandoNovamente, onTentarNovamente }) {
  ...
  const mostrarMotivoErro = (statusNormalizado === 'erro' || saidaComErro) && Boolean(processamento.motivo_erro);
  ...
  return (
    <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      {saidaComErro ? (
        <p className="mt-3 text-sm font-semibold text-rose-800">Falha ao gerar arquivos</p>
      ) : null}
      ...
      {saidaComErro ? (
        <button
          type="button"
          onClick={() => onTentarNovamente(processamento.id)}
          disabled={estaTentandoNovamente}
          className="mt-2 inline-flex items-center justify-center gap-1.5 rounded-md border border-rose-300 bg-white px-2.5 py-1.5 text-xs font-semibold text-rose-700 transition hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-60"
        >
          ...
          <section className="grid gap-3 lg:hidden">
            {processamentos.map((processamento) => (
              <ProcessamentoCard
                ...
                onTentarNovamente={handleTentarNovamenteSaida}
              />
            ))}
          </section>
