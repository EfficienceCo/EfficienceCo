 ...
 *  - a célula "BASE IRRF" do rodapé exibe a base real mesmo quando a retenção
 *    é zerada pela faixa de isenção/redução;
...
describe("BUG-FOLHA-02 — base de cálculo do IRRF no holerite", () => {
  it("calcularFolhaFuncionario retorna a base_ir efetivamente usada pelo IRRF", () => {
    const entrada = linha({ salario_bruto: 6000, num_dependentes: 2 });
    const calculo = calcularFolhaFuncionario(entrada, COMPETENCIA);

    assert.ok("base_ir" in calculo, "base_ir precisa estar no retorno para ser persistido");

    const tabela = resolverTabelaFolha(COMPETENCIA);
    const esperado = calcularBaseIR({
      baseCalculo: calculo.base_calculo,
      inss: calcularINSS(calculo.base_calculo, tabela.inss),
      numDependentes: entrada.num_dependentes,
    }, tabela.irrf);
  ...
  it("holerite com IRRF > 0 mostra BASE IRRF coerente com o cálculo (não '-')", () => {
    const calculo = calcularFolhaFuncionario(linha({ salario_bruto: 6000 }), COMPETENCIA);
  ...
  it("BASE IRRF é exibida mesmo quando a retenção é zero", () => {
    const calculo = calcularFolhaFuncionario(linha({ salario_bruto: 1600 }), COMPETENCIA);

    assert.equal(calculo.ir, 0, "salário baixo não deve reter IRRF (pré-condição do teste)");
    assert.equal(celulaBaseIrrf(calculo).texto, formatarMoeda(calculo.base_ir));
  });

  it("dependentes podem zerar o IRRF sem apagar a base usada", () => {
    const calculo = calcularFolhaFuncionario(
      linha({ salario_bruto: 3000, num_dependentes: 8 }),
      COMPETENCIA,
    );
  ...
  it("linha legada sem base_ir persistido (pré-migration 89) mostra '-', não '0,00'", () => {
    const calculoLegado = {
      salario_bruto: 5000,
      valor_horas_extras: 0,
      valor_faltas: 0,
