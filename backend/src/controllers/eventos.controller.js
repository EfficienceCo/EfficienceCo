...
export async function registrarEvento(req, res) {
  ...
  if (!cliente_id || !descricao || sucesso === undefined) {
    return res.status(400).json({ erro: 'Campos obrigatórios: cliente_id, descricao, sucesso' });
  }
  ...
  console.log('[eventos.controller] Registrando evento para cliente:', cliente_id);
  ...
  if (error) {
    console.error('[eventos.controller] Erro ao registrar evento:', error.message);
