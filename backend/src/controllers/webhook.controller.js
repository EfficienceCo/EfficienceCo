...
async function atualizarLicenca(clienteId, ativa) {
  console.log(
    `[webhook.controller] ${ativa ? "Ativando" : "Desativando"} licença do cliente: ${clienteId}`,
  );
  ...
  if (error) {
    console.error(
      `[webhook.controller] Erro ao atualizar licença do cliente ${clienteId}:`,
      error.message,
    );
  } else {
    console.log(
      `[webhook.controller] Licença do cliente ${clienteId} ${ativa ? "ativada" : "desativada"} com sucesso`,
    );
  }
}
...
export async function processarWebhook(req, res) {
  ...
  try {
    ...
    return res
      .status(400)
      .json({ erro: `Assinatura inválida: ${err.message}` });
  }
    ...
    switch (evento.type) {
      ...
      case "checkout.session.completed": {
        ...
        if (clienteId) {
          ...
          console.log(
            "[webhook.controller] checkout.session.completed sem cliente_id no metadata — ignorado",
          );
        }
        break;
      }
      ...
      case "customer.subscription.deleted": {
        ...
        if (clienteId) {
          ...
          console.log(
            "[webhook.controller] customer.subscription.deleted sem cliente_id no metadata — ignorado",
          );
        }
        break;
      }
      ...
      case "invoice.payment_failed": {
        ...
        console.log(
          `[webhook.controller] Falha de pagamento na fatura: ${invoice.id}`,
        );
        ...
        if (invoice.subscription) {
          ...
          if (clienteId) {
            ...
            console.log(
              "[webhook.controller] invoice.payment_failed — subscription sem cliente_id no metadata",
            );
          }
        }
        break;
      ...
      default:
        console.log(
          "[webhook.controller] Evento ignorado (não tratado):",
          evento.type,
        );
    }
  } catch (err) {
    console.error(
      "[webhook.controller] Erro ao processar evento:",
      err.message,
    );
